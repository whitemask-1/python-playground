...
def print_movie_list():
    ...
    for i, (title, info) in enumerate(movies.items(), start=1):
        status_icon = (
            watched_indicator if info["status"] == "watched" else to_watch_indicator
        )
        print(
            f"{i} | {status_icon} | {title} | [{info['genre']}] - Rating: {info['rating']}"
        )
...
def toggle_rating(movie_title):
    ...
    if movies[movie_title]["status"] != "watched":
        print("You can only rate movies marked as watched.")
        return
    rating = input("Enter a new rating (1-5): ").strip()
    if rating in ["1", "2", "3", "4", "5"]:
        movies[movie_title]["rating"] = "✦" * int(rating)
        print(
            f'✓ Updated rating for "{movie_title}" to {movies[movie_title]["rating"]}'
        )
        save_movies()
        print_movie_list()
    else:
...
def add_movies():
    ...
    movies[movie] = {"status": status, "genre": genre, "rating": rating}
...
def view_movies():
    ...
    if filter_choice == "2":
        filtered_movies = {
            title: info for title, info in movies.items() if info["status"] == "watched"
        }
...
def show_movie_stats():
    ...
    print(
        f"|| Watched Percentage: { (watched_movies / total_movies * 100) if total_movies > 0 else 0:.2f}% "
